...
type ExamBatch struct {
	...
	BatchKind             int         `json:"batch_kind"                   orm:"batch_kind"                   description:"0=formal 1=practice"`                 // 0=formal 1=practice
	AllowMultipleAttempts int         `json:"allow_multiple_attempts"      orm:"allow_multiple_attempts"      description:"1=同用户同卷可多条会话"`                        // 1=同用户同卷可多条会话
	MaxAttemptsPerMember  int         `json:"max_attempts_per_member"      orm:"max_attempts_per_member"      description:"可重复时每人每卷上限，0=不限制"`                    // 可重复时每人每卷上限，0=不限制
	SkipScoring           int         `json:"skip_scoring"                 orm:"skip_scoring"                 description:"1=不落正式成绩"`                            // 1=不落正式成绩
	AutoSubmitOnDeadline  int         `json:"auto_submit_on_deadline"      orm:"auto_submit_on_deadline"      description:"0=不因个人 deadline 自动交卷/拦截保存，批次过期也不代交卷"` // 0=不因个人 deadline 自动交卷/拦截保存，批次过期也不代交卷
